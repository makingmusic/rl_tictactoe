...
class TicTacToe:
    ...
    def make_move(self, row, col, whose_turn=None):
        ...
        if whose_turn is not None and whose_turn != self.next_player:
            raise WrongMoveError(f"Turn mismatch. Expected: {self.next_player}, but got: {whose_turn}")
    ...
    def check_win(self):
        ...
        for col in range(self.size):
            if [self.board[row][col] for row in range(self.size)].count(STRING_X) == self.size:
                return STRING_X
            if [self.board[row][col] for row in range(self.size)].count(STRING_O) == self.size:
                return STRING_O
        ...
        # if no player has won the game, return STRING_WINNER_NONE 
        return None
